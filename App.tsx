...
import { generateProPhoto } from './services/gemini';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initHistory = async () => {
      // Changed key to separate from previous app version
      const saved = localStorage.getItem('gemini_photo_history');
  ...
  useEffect(() => {
    if (history.length > 0) {
        try {
            localStorage.setItem('gemini_photo_history', JSON.stringify(history));
        } catch (e) {
  ...
  const handleGenerate = async (promptText: string, category: string, file?: File) => {
    ...
    try {
      ...
      const generatedImage = await generateProPhoto(promptText, category, imageBase64, mimeType);
      
      if (generatedImage) {
        const newCreation: Creation = {
          id: crypto.randomUUID(),
          name: file ? `${category.charAt(0).toUpperCase() + category.slice(1)}` : `Génération ${category}`,
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
        try {
            ...
            // Basic validation for photo format
            if (parsed.generatedImage && parsed.name) {
  ...
  return (
    <div className="h-[100dvh] bg-zinc-950 bg-dot-grid text-zinc-50 selection:bg-purple-500/30 overflow-y-auto overflow-x-hidden relative flex flex-col">
      ...
      <div className="fixed bottom-4 right-4 z-50">
        ...
        >
            <span className="text-xs font-medium uppercase tracking-wider hidden sm:inline">Importer JSON</span>
