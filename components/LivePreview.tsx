...
import { ArrowDownTrayIcon, PlusIcon, ViewColumnsIcon, DocumentIcon, CameraIcon, XMarkIcon } from '@heroicons/react/24/outline';
  ...
  useEffect(() => {
    const renderPdf = async () => {
      ...
      try {
        ...
        setError("Impossible d'afficher l'aperçu PDF.");
        setLoading(false);
      }
    };
...
export const LivePreview: React.FC<LivePreviewProps> = ({ creation, isLoading, isFocused, onReset }) => {
    ...
    const handleDownload = () => {
        ...
        link.download = `${creation.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_pro_photo.png`;
      ...
      <div className="bg-[#121214] px-4 py-3 flex items-center justify-between border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center space-x-2 text-zinc-500">
            ...
            <span className="text-[11px] font-mono uppercase tracking-wider">
                {isLoading ? 'Traitement en cours...' : creation ? creation.name : 'Mode Studio'}
            </span>
        </div>
                ...
                <>
                    {creation.originalImage && (
                         <button 
                            ...
                            title={showSplitView ? "Voir le résultat seul" : "Comparer avec l'original"}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 w-full">
             ...
             <div className="w-full max-w-md space-y-8">
                <div className="flex flex-col items-center">
                    <div className="w-12 h-12 mb-6 text-purple-500 animate-pulse">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                           <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                        </svg>
                    </div>
                    <h3 className="text-zinc-100 font-mono text-lg tracking-tight">Traitement de la composition</h3>
                    <p className="text-zinc-500 text-sm mt-2">Génération des détails haute résolution...</p>
                </div>
                ...
                <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden">
                 ...
                 <div className="border border-zinc-800 bg-black/50 rounded-lg p-4 space-y-3 font-mono text-sm">
                     <LoadingStep text="Analyse de la structure visuelle" active={loadingStep === 0} completed={loadingStep > 0} />
                     <LoadingStep text="Amélioration de la lumière et des textures" active={loadingStep === 1} completed={loadingStep > 1} />
                     <LoadingStep text="Rendu des détails photoréalistes" active={loadingStep === 2} completed={loadingStep > 2} />
                     <LoadingStep text="Finalisation du rendu" active={loadingStep === 3} completed={loadingStep > 3} />
                 </div>
             </div>
          </div>
        ) : creation?.generatedImage ? (
          <>
                    ...
                    <div className="w-full h-full p-6 flex items-center justify-center overflow-hidden">
                        {creation.originalImage.startsWith('data:application/pdf') ? (
                            ...
                            <img 
                                ...
                                alt="Original Input" 
                                className="max-w-full max-h-full object-contain shadow-xl border border-zinc-800/50 rounded opacity-80"
                            />
                        )}
                    </div>
                </div>
            ...
            <div className={`relative h-full bg-[#050505] flex items-center justify-center transition-all duration-500 ${showSplitView && creation.originalImage ? 'w-full md:w-1/2 h-1/2 md:h-full' : 'w-full'}`}>
                 <img
                    src={creation.generatedImage}
                    alt="Generated Pro Photo"
